...
def downsampleAudio(inputFile, M):
    ...
    """
    (fs, wavx) = wavread(inputFile)
    
    wavx_downSampling = hopSamples(wavx, M)
    
    downSamplingRate = int(fs / M)
    
    print (downSamplingRate)
    fileName = inputFile.replace(".wav"," downsampled.wav")
    wavwrite(wavx_downSampling, downSamplingRate ,fileName)
    return 
