...
def testRealEven(x):
    ...
    '''
    If f is real, jF[k]j = jF[N − k]j and <F[k] = − <F[N − k]
    If f is real and even, jF[k]j = jF[N − k]j and imag(F[k]) = 0
    '''
    
    isComplexArray = np.iscomplex(np.round(X,6))
    
    isRealEven = np.all(isComplexArray == False)
    
    outIsRealEven = False # convert numpy.bool_ to bool , to be accepted in the tester
    if isRealEven:
        outIsRealEven = True
       
    
    return (outIsRealEven, dftbuffer, X)
