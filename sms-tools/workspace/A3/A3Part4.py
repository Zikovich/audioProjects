...
def suppressFreqDFTmodel(x, fs, N):
    ...
    ## Your code here
    (myAnal, pyAnal) = dftAnal(x, w, N)
    
    y = dftSynth(myAnal, pyAnal, M)*outputScaleFactor
    
    # normalize bins  below 70 Hz to -120 DB
    
    binHz = int(math.ceil(70 * N / fs))
    
    myAnalFilt = np.copy(myAnal)
    pyAnalFilt = np.copy(pyAnal)
    
    myAnalFilt[:binHz+1] = np.repeat(-120, binHz+1)
    #pyAnalFilt[:binHz] = np.repeat(0, binHz)
    ...
    yfilt = dftSynth(myAnalFilt, pyAnalFilt, M)*outputScaleFactor
